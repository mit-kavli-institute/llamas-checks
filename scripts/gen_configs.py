...
Design (2026-09 revision -- see docs/QA_CHECKS_CATALOGUE.md):
...
- FAIL is reserved for unambiguous defects; anything that is merely "high" is WARN.
  Two-tier checks are two rules (like ccd_temperature_warm/_hot):
    * uniform frames (bias/dark): row/column structure WARN at the per-detector cap,
      FAIL (``*_gross``) at STRUCT_FAIL_FACTOR x cap; saturation WARN at the cap,
      FAIL (``saturation_gross``) above SAT_GROSS_UNIFORM;
    * illuminated frames: structure FAIL at the cap only where the lamp is fixed
      (LDLS, ThAr) and WARN on twilight sky flats (brightness varies by design);
    * every frame type (incl. science): ``edge_saturated`` FAIL when the
...
Reads --derived (default: the package baselines/qa_thresholds_derived.json) and
writes the two YAMLs into --out-dir (default: the package configs/ directory).
"""
import argparse
import json
import os
...
DER = None   # derived-threshold dict; loaded in main() (helpers below read it)

# --- severity-tier constants (2026-09) ---------------------------------------
# Gross structure FAIL on uniform frames = this multiple of the per-detector WARN cap.
# Calibration on real frames (2 %-trimmed-mean profile metric): the 2026-06-04 odd dark
# (2.B.Green full-height bars) sits at ~6.6x its cap; benign glow gradients / variable
# fixed patterns on 2026-09 biases and darks stay below ~3x.
STRUCT_FAIL_FACTOR = 4.0
# Fraction of a bias/dark frame above the saturation level that is a hard FAIL
# (light leak / railed readout). Baseline maximum is 4.8e-4.
SAT_GROSS_UNIFORM = 0.01
# Unilluminated bottom-stripe median at/above this = the stripe itself is saturated.
# Normal frames (incl. the 0.3 s LDLS flats that rail the reds) stay below ~5000 ADU.
EDGE_SAT_ADU = 63000.0
...
def base_blocks():
    return {
        ...
        "metrics": {
            "median": {"type": "median"},
            "rms": {"type": "robust_std"},   # 1.4826*MAD: read noise, blind to hot pixels
    ...
    if gross:
        rules += [
            {"name": "row_structure_gross", "region": "full_frame", "metric": "row_banding",
             ...
             "expected_from_lookup": {"table": table, "keys": KEYS, "max_field": "row_fail_max"}},
            {"name": "column_structure_gross", "region": "full_frame", "metric": "column_banding",
             "per_extension": True, "severity": "FAIL",
             "expected_from_lookup": {"table": table, "keys": KEYS, "max_field": "col_fail_max"}},
        ]
    return rules
...
def sat_gross_rule():
    """Uniform frames only: >= SAT_GROSS_UNIFORM of the frame saturated is a hard FAIL."""
    return {"name": "saturation_gross", "region": "full_frame", "metric": "saturated_fraction",
            "per_extension": True, "severity": "FAIL", "limits": {"max": SAT_GROSS_UNIFORM}}


def edge_sat_rule():
    """Any frame type: the unilluminated bottom stripe at the ADC ceiling is a hard FAIL."""
    return {"name": "edge_saturated", "region": "bottom_stripe", "metric": "median",
            "per_extension": True, "severity": "FAIL", "limits": {"max": EDGE_SAT_ADU}}
...
def build_cal_config():
    ...
    # (PRODCATG, set_name, uniform?, structure severity at the cap)
    ...
    types = [("CAL.R-BIA", "BIAS", True, "WARN"), ("CAL.R-DRK", "DARK", True, "WARN"),
             ("CAL.R-FLT", "LDLS_FLAT", False, "FAIL"), ("CAL.R-SKY", "SKY_FLAT", False, "WARN"),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate qa_config_cal.yaml and qa_config_science.yaml from the derived thresholds.")
    parser.add_argument("--derived", default=str(BASELINES_DIR / "qa_thresholds_derived.json"),
                        help="derived-thresholds JSON from aggregate_thresholds.py "
                             "(default: the package baselines/qa_thresholds_derived.json)")
    parser.add_argument("--out-dir", default=str(CONFIG_DIR),
                        help="where to write the two YAMLs (default: the package configs/ directory)")
    args = parser.parse_args()
    DER = json.load(open(args.derived))
