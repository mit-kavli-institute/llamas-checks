...
`llamas-checks-engine` (`llamas_checks.qa_engine`) and validated by `llamas-checks-validate`
(`llamas_checks.qa_config_validator`). The package is standalone (`llamas-checks`); it does not
...
```bash
llamas-checks-engine /path/to/night_dir --config qa_config_cal.yaml     # batch, parallel
llamas-checks-engine LLAMAS_..._SCI22_mef.fits --config qa_config_science.yaml
```
A bare config name (`qa_config_cal.yaml`) resolves to the copy shipped in the package
`configs/` directory; a path is used as given. Each file gets a `<name>.qa.json` report. **Exit codes:** PASS/WARN → 0, FAIL → 1,
...
### `llamas-checks` — user-facing single-file front-end

`llamas-checks` (`llamas_checks.QA_assess`; `python -m llamas_checks <file>` is an alias) is the
interface that links QA results to users (the observing GUI consumes its exit code and `--report`
JSON). It runs the same engine on **one** image but **auto-selects the config from the frame's
`PRODCATG`** — `CAL.*` → `qa_config_cal.yaml`, `SCI.*` → `qa_config_science.yaml` — so you do not
pass `--config`:
...
`3`** even when quiet, so a broken invocation stays diagnosable.

**Structure block.** Every `check_image` result — and therefore every `--report` JSON — carries
`result["structure"]`, a header-only inspection of the MEF layout made before the engine runs
(`llamas_checks.validate.inspect_structure`). Its keys: `n_extensions` (image extensions present,
`len(hdul) - 1`), `expected_extensions` (24), `missing_cameras` (detectors such as `"1.A.Blue"` whose
...
Artifacts (`llamas_checks/baselines/`): `qa_thresholds_derived.json` (limits + observed stats)
and `qa_tracking_baselines.csv` (per-epoch medians for **drift monitoring**); the reproducible
pipeline is `scripts/` at the repository root (`sort_baselines`, `extract_stats`,
`aggregate_thresholds`, `gen_configs`). Re-run to refresh thresholds as more nights arrive; the
YAMLs are generated, so edit `scripts/gen_configs.py` (or the YAML) and re-validate with
`llamas-checks-validate`.
...
`llamas_checks/baselines/qa_thresholds_derived.json` (`per_detector`); the shutter tolerances and the
...
run also writes `<name>.qa.json` next to the input. (Frames and the engine geometry are both
2048×2048 per detector; the derived thresholds were built on this geometry, so region slices and
limits stay consistent. Deliberate correction in this port: the pipeline copy of this document
said "overscan-trimmed to 2028×2048", which does not match the raw frames or the region slices.)

### Targeted test cases — `llamas-checks-engine <file> --config <cfg> --summary-only`
...
synthetic +8/+15 header injection WARNs then FAILs (`tests/test_qa_engine_header.py`). The doc's
...
`python scripts/run_qa_tests.py` re-runs the targeted 15-case matrix (prints the exit code +
fired rules per case, writes `./qa_test_results.json`; because it drives the engine CLI it writes a
`.qa.json` next to each input, on purpose); `python scripts/batch_tally.py` re-runs the full
154-file sweep in-process (no report writes; `./batch_tally.json`); `pytest tests/` runs the unit
tests (shutter, per-detector warm/hot, keyword variants, structure inspection, CLI). Thresholds
regenerate via `python scripts/extract_stats.py` → `python scripts/aggregate_thresholds.py` →
`python scripts/gen_configs.py` (re-validate with `llamas-checks-validate`). The scripts take
their data roots from arguments or environment variables — `--baselines-root` /
`LLAMAS_QA_BASELINES` (the `QA_baselines` folder holding `copies/ Bias/ Darks/ Arcs/ lamp_flats/
twilight_flats/`), `--warm-dir` / `LLAMAS_QA_WARM_DIR` (the 2026-05-06 warm-incident frames) and
`--commissioning-dir` / `LLAMAS_QA_COMMISSIONING_DIR` (`ut20260710_11`); a case whose root is unset
is reported as `MISSING FILE` and skipped. See the README section "Regenerating thresholds".
