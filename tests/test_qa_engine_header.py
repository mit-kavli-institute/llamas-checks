...
def run(path):
    return E.QAEngine(CONFIG).run(Path(path))
...
def test_off_mode_lookup_miss_skips_not_crashes(tmp_path):
    ...
    rep = E.QAEngine(cfg).run(Path(path))
...
def test_no_rules_matched_warns(tmp_path):
    ...
    rep = E.QAEngine(CONFIG).run(Path(path))
    assert rep["overall_verdict"] == "WARN"
...
def _run(cfg, path):
    return E.QAEngine(cfg).run(Path(path))
